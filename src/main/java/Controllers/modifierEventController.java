        ...
        try {
            ...
            if (activiteCombo != null && event.getIdActivite() > 0) {
                for (int i = 0; i < activitesList.size(); i++) {
                    if (activitesList.get(i) instanceof Entities.Activite) {
                        Entities.Activite activite = (Entities.Activite) activitesList.get(i);
        ...
        if (selectedTitre != null && !selectedTitre.trim().isEmpty()) {
            try {
                for (Object activiteObj : activitesList) {
                    if (activiteObj != null && activiteObj instanceof Entities.Activite) {
                        Entities.Activite currentActivite = (Entities.Activite) activiteObj;
